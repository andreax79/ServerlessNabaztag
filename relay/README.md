...
The relay transcribes button audio with the configured ISO language and a short device-vocabulary hint, then classifies a small allow-list of explicit, reversible intents (ears, LEDs, sounds, and live rabbit status). It creates that exact function call in the Realtime conversation and waits for the rabbit's result before asking the model to answer. Ordinary conversation and the final answer after a tool result both use `tool_choice: "none"`; `rabbit_status` is never a preliminary step or fallback. The device pipeline returns `ok` only after checking the resulting state: LED overrides are read back in Forth, while ear movement is started immediately and verified by the relay against the calling rabbit's live `/status` endpoint. Otherwise an `error:` result is sent back to the model. This extra transcription and verification add some latency and cost, but prevent a physical request from being acknowledged without executing the corresponding tool.
